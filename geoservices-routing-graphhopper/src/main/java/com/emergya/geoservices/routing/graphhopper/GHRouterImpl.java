...
public class GHRouterImpl implements GHRouter {
    ...
    public FindShortestPathResponse createRoute(FindShortestPath parameters) {
        ...
        while (instructions.hasNext() && i < size - 1) {
            ...
            for (int j = 0; j < numberEdges; j++) {
                ...
                GHPoint p2;
                Geometry geom = null;
                String srs = parameters.getStartPoint().getPoint().getSrsName();
                ...
                if (j < numberEdges) {
                    ...
                    try {
                        ...
                        else if (i == (size - 2) && j == (numberEdges - 1)) {
                            targetEdge = geom;
                        }
                ...
                String geometry = writerGeom.write(geom);
        ...
        Coordinate iPointSP = new Coordinate(startEdge.getCoordinates()[0].x, startEdge.getCoordinates()[0].y);
        ...
        Coordinate fPointSP = new Coordinate(startEdge.getCoordinates()[1].x, startEdge.getCoordinates()[1].y);
        ...
        Coordinate iPointTP = new Coordinate(targetEdge.getCoordinates()[0].x, targetEdge.getCoordinates()[0].y);
        ...
        Coordinate fPointTP = new Coordinate(targetEdge.getCoordinates()[1].x, targetEdge.getCoordinates()[1].y);
    ...
    private GHPlace toGHPlace(Point p) throws FactoryException, MismatchedDimensionException, TransformException {
        ...
        Coordinate c = new Coordinate(coordinates[1], coordinates[0]);
        if (coordInicio == null) {
            coordInicio = c;
        } else {
            coordFinal = c;
